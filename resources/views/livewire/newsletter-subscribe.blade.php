<div x-data x-on:pageshow.window="if ($event.persisted) { $wire.successMessage = ''; $wire.errorMessage = ''; }">
    ...
    @else
        <form wire:submit="subscribe" novalidate>
            ...
            @if ($variant === 'card')
                {{-- Card variant: label left + input with arrow button --}}
                <div class="flex items-center gap-4">
                    <label for="newsletter-email" class="shrink-0 text-xl font-bold text-neutral-900 dark:text-neutral-100">{{ __('Newsletter') }}</label>
