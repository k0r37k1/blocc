<div
    x-data="{
        ...
        insertEmoji(emoji) {
            ...
            if (editorEl?.__tiptapEditor) {
                editorEl.__tiptapEditor.chain().focus().insertContent(emoji).run();
            } else if (editorEl) {
                editorEl.focus();
    ...
    x-init="
        _observer = new MutationObserver(() => { isDark = document.documentElement.classList.contains('dark') });
...
>
    ...
    <div
        x-ref="picker"
        x-show="showEmojis"
        x-effect="if (showEmojis) $nextTick(() => $refs.picker.style.display = 'grid')"
